...
void DbusPidZone::initializeCache(void)
{
    ...
    /*
     Currently, find the max failsafe percent pwm settings from zone and controller,
     and assign it to zone failsafe percent.
    */
    std::cerr << "zone: Zone" << _zoneId << " failSafePercent: " << _failSafePercent <<"\n";
    for (auto it = _pidsFailSafePercent.begin(); it != _pidsFailSafePercent.end(); it++)
    {
    ...
    std::cerr << "Final zone" << _zoneId << " failSafePercent: " << _failSafePercent <<"\n";
}
