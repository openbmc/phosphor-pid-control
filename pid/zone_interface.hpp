...
class ZoneInterface
{
  public:
     ...
     * A compatibility interface is implemented here, which just discards.
     */
     ...
     */
    virtual std::pair<double, double> getCachedValues(const std::string& name)
    {
        return std::make_pair(getCachedValue(name), getCachedValue(name));
    }
