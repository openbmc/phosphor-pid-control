...
// Redundant log entries once every 60 seconds
static constexpr int logThrottle = 60 * 1000;

static std::map<std::string, PidLog> nameToLog;
...
static void DumpPidCoreHeader(std::ofstream& file)
{
    ...
    file << ",boundsOut";
    file << ",output3,integralTerm3";
    file << ",integralTerm,output";
    file << "\n" << std::flush;
}

static void DumpPidCore(std::ofstream& file,
                        ...
                        const PidCore& pc)
{
    ...
    file << "," << pc.boundsOut;
    file << "," << pc.output3 << "," << pc.integralTerm3;
    file << "," << pc.integralTerm << "," << pc.output;
    file << "\n" << std::flush;
}

static void DumpPidCoeffsHeader(std::ofstream& file)
{
...
static void DumpPidCoeffs(std::ofstream& file,
                          const std::chrono::milliseconds& msNow,
...
void LogInit(const std::string& name, pid_info_t* pidinfoptr)
{
    ...
    else
    {
        ...
        DumpPidCoreHeader(iterExisting->second.filePid);
        DumpPidCoeffsHeader(iterExisting->second.fileCoeffs);
                ...
                const PidCore& coreLog)
{
    ...
    pidLog.lastCore = coreLog;

    DumpPidCore(pidLog.filePid, msNow, coreLog);
}
