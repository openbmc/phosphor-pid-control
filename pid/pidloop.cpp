...
void pidControlLoop(std::shared_ptr<ZoneInterface> zone,
                    ...
                    const bool* cancel, bool first, int ms100cnt)
{
    if (*cancel)
        return;
    ...
    timer->async_wait([zone, timer, ms100cnt,
                       cancel](const boost::system::error_code& ec) mutable {
        ...
        {
            pidControlLoop(zone, timer, cancel, false, ms100cnt);
            return;
        }
