...
void from_json(const json& j, conf::ControllerInfo& c)
{
    ...
    else
    {
        ...
        {
            c.stepwiseInfo.reading[i] = std::numeric_limits<double>::quiet_NaN();
        ...
        {
            p.at("positiveHysteresis").get_to(c.stepwiseInfo.positiveHysteresis);
        }
        ...
        {
            p.at("negativeHysteresis").get_to(c.stepwiseInfo.negativeHysteresis);
        }
    }
}
} // namespace conf
