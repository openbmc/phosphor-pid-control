...
class HostStateMonitor
{
  public:
    ...
    void stopMonitoring();
    void getInitialState(const std::string& serviceName);
  ...
  private:
    ...
    static uint64_t parseHostIdFromPath(const std::string& path);
        ...
        std::unordered_map<std::string, std::variant<std::string>>>& interfaces)
{
    ...
    try
    {
        hostId = parseHostIdFromPath(pathStr);
    }
    catch (const std::exception& e)
    {
        std::cerr << "Unable to parse hostId from path " << pathStr << ": "
                  << e.what() << std::endl;
        return;
    }

    if (instances.contains(hostId))
    {
        return;
    }

    auto monitor = std::make_unique<HostStateMonitor>(bus, pathStr, hostId);
    ...
    instances.emplace(hostId, std::move(monitor));
}
...
inline void HostStateMonitor::discoverExistingHosts(sdbusplus::bus_t& bus)
{
    ...
    try
    {
        ...
        respData = resp.unpack<decltype(respData)>();
    }
    catch (const sdbusplus::exception_t& e)
    {
        std::cerr << "Failed to discover existing hosts: " << e.what()
                  << std::endl;
        return;
    }

    for (const auto& [pathStr, services] : respData)
    {
        if (!pathStr.starts_with(hostStatePath))
        {
            continue;
        ...
        if (services.empty())
        {
            std::cerr << "No services found for host path " << pathStr
                      << std::endl;
            continue;
        }
        ...
        monitor->getInitialState(serviceName);
        instances.emplace(hostId, std::move(monitor));
    }
}
...
inline void HostStateMonitor::stopAll()
{
    ...
    for (auto& [hostId, monitor] : instances)
    {
        if (monitor)
        {
...
inline uint64_t HostStateMonitor::parseHostIdFromPath(const std::string& path)
{
    return std::stoull(path.substr(std::strlen(hostStatePath)));
}

inline std::optional<std::reference_wrapper<HostStateMonitor>>
    HostStateMonitor::getInstance(uint64_t hostId)
{
    auto it = instances.find(hostId);
    if (it == instances.end())
    {
        return std::nullopt;
    }
...
inline HostStateMonitor::HostStateMonitor(
    sdbusplus::bus_t& bus, const std::string& hostObjectPath, uint64_t hostId) :
    bus(bus), hostId(hostId), hostPath(hostObjectPath), powerStatusOn(false),
...
inline void HostStateMonitor::handleStateChange(sdbusplus::message_t& message)
{
    ...
    {
        std::cerr << "Failed to handle host " << hostId
                  << " state change: " << e.what() << std::endl;
    }
}

inline void HostStateMonitor::getInitialState(const std::string& serviceName)
{
    try
    {
        auto method = bus.new_method_call(serviceName.c_str(), hostPath.c_str(),
    ...
    {
        std::cerr << "Failed to get initial host state for host " << hostId
                  << ": " << e.what() << std::endl;
        powerStatusOn = false;
    }
}
