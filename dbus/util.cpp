...
using Value = sdbusplus::message::variant<double, int64_t, std::string>;
                               ...
                               struct SensorProperties* prop)
{
    ...
    auto findValue = propMap.find("Value");
    if (findScale != propMap.end() && findValue != propMap.end())
    {
        // if scale is avaliable, means value is an int64_t
        prop->scale =
            sdbusplus::message::variant_ns::get<int64_t>(findScale->second);
        prop->value =
            sdbusplus::message::variant_ns::get<int64_t>(findValue->second);
    }
    else if (findValue != propMap.end())
    {
        // if scale is unavaliable, means value is double
        prop->scale = 0;
        prop->value =
            sdbusplus::message::variant_ns::get<double>(findValue->second);
    }
    else
    {
        throw std::runtime_error(path + " has invalid properties\n");
    }
