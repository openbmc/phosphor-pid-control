    ...
    for (const auto& configuration : configurations)
    {
        ...
        if (findStepwise != configuration.second.end())
        {
            ...
            for (const std::string& zone : zones)
            {
                ...
                std::fill(std::begin(info.stepwiseInfo.reading),
                          std::end(info.stepwiseInfo.reading), 0);
                ...
                std::fill(std::begin(info.stepwiseInfo.output),
                          std::end(info.stepwiseInfo.output), 0);
