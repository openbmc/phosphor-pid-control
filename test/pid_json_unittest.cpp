...
TEST(ZoneFromJson, getCycleInterval)
{
    ...
    std::tie(pidConfig, zoneConfig) = buildPIDsFromJson(j2);
    EXPECT_EQ(pidConfig.size(), 1U);
    EXPECT_EQ(zoneConfig.size(), 1U);

    EXPECT_EQ(pidConfig[1]["fan1-5"].type, "fan");
    EXPECT_EQ(zoneConfig[1].cycleTime.cycleIntervalTimeMS, 1000ULL);
    EXPECT_EQ(zoneConfig[1].cycleTime.updateThermalsTimeMS, 1000ULL);
    EXPECT_DOUBLE_EQ(zoneConfig[1].minThermalOutput, 3000.0);
}
