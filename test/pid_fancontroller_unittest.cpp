...
TEST(FanControllerTest, OutputProc_VerifyFailSafeWhenIdenticalPercent)
{
    // Test that when in failsafe and the calculated outputProc is the same as
    // the failsafe PWM, the failsafe PWM is set.
    ...
    // This is a fan PID, so calling outputProc will try to write this value
    // to the sensors.
    p->outputProc(failsafepercent);
}

TEST(FanControllerTest, OutputProc_VerifyStrictFailSafeWhenIdenticalPercent)
{
    // Test that when in failsafe and the calculated outputProc is the same as
    // the failsafe PWM, the failsafe PWM is set. Verify that this holds true
    // even with the STRICT_FAILSAFE_PWM flag enabled.
