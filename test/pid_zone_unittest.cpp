...
TEST_F(PidZoneTest, ThermalInputTest) {
    ...
    // nothing cares about the type presently, this is just for huamns.
    std::string type = "unchecked";

    mgr.addSensor(type, name1, std::move(sensor1));
    EXPECT_EQ(mgr.getSensor(name1).get(), sensor_ptr1);
    mgr.addSensor(type, name2, std::move(sensor2));
    EXPECT_EQ(mgr.getSensor(name2).get(), sensor_ptr2);
    ...
    // GetTimeout calls into the base class.
...
TEST_F(PidZoneTest, FanInputTest) {
...
TEST_F(PidZoneTest, ThermalInputTestFailSafeMode) {
    ...
    // sensor1 will be bad, but sensor2 will be fine. :D
...
TEST_F(PidZoneTest, GetSensorTest) {
    ...
    EXPECT_EQ(mgr.getSensor(name1).get(), zone->getSensor(name1).get());
}

TEST_F(PidZoneTest, AddThermalPIDTest) {
    ...
    // Kind of wonky, you need to do it this way. ;)
...
TEST_F(PidZoneTest, AddFanPIDTest) {
...
TEST_F(PidZoneTest, ManualModeDbusTest) {
...
TEST_F(PidZoneTest, FailsafeDbusTest) {
