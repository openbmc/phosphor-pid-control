...
TEST_F(PidZoneTest, GetFailSafePercent_SingleFailedReturnsExpected)
{
    ...
    std::string finalFailSafeName = "";

    zone->markSensorMissing("temp1");
    zone->getFailSafePercent(finalFailSafeName, finalFailSafePercent);

    EXPECT_EQ("", finalFailSafeName);
    EXPECT_EQ(failSafePercent, finalFailSafePercent);
}
...
TEST_F(PidZoneTest, GetFailSafePercent_MultiFailedReturnsExpected)
{
    ...
    {
        std::string sensorName = "temp" + std::to_string(num);
        EXPECT_EQ(sensorName, name);
        num++;
    }
    EXPECT_EQ(80, finalFailSafePercent);
}
