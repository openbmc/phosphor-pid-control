...
TEST(PidZoneTest, BoringConstructorTest) {
    ...
    std::shared_ptr<SensorManager> m =
        std::make_shared<SensorManager>(std::move(bus_mock_passive),
    ...
    PIDZone p(zone, minThermalRpm, failSafePercent, m, bus_mock_mode, objPath, defer);
    // Success.
}
